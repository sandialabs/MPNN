...
class MPNN():
    ...
    def set_rhombi(self):
        ...
        for j in range(self.ncl):
            ...
            #             delta_x=1.0,
            #             delta_z=1.0)
            # xyfold = False # TODO be careful! evaluation in SModel always folds!
    ...
    def eval(self, xx, eps=0.02, temp=None, eval_type=None, krnl='exp'):
        ...
        wfcn = WFcn(self.mpts, eps, rhomb=self.rhomb_eval, krnl=krnl)

        models = [SModel(ptmodel, mpt, rhomb, expon=self.expon) for ptmodel, mpt, rhomb in zip(self.ptmodels, self.mpts, self.rhombi)]
